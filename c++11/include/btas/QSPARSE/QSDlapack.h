...
(const BTAS_ARROW_DIRECTION& ArrowDir,
 const QSDArray<2, Q>& a, SDArray<1>& s, QSDArray<2, Q>& u, QSDArray<2, Q>& vt)
{
  int nrows = a.shape(0);
  int ncols = a.shape(1);
  ...
  for(typename QSDArray<2, Q>::const_iterator ia = a.begin(); ia != a.end(); ++ia) {
    ...
    DgesvdArglist<2, 2> gesvd_list(ia->second, is->second, iu->second, iv->second);
...
//QSDArray<2, Q> vt_merge(vt_q_total, vt_q_mshape);
  ...
  // Containers of truncated quantum number indices and sizes
  Qshapes<Q> q_sval_tr; q_sval_tr.reserve(n_sval);
  ...
  SDArray<1> s_value_tr(shape(nnz));
...
//TVector<Qshapes<Q>, 2> u_q_mshape_tr = { q_rows,-q_sval_tr };
//QSDArray<2> u_merge_tr(u_merge.q(), u_q_mshape_tr);
  QSDArray<2> u_merge_tr(u_merge.q(), make_array( q_rows,-q_sval_tr));
...
//TVector<Qshapes<Q>, 2> vt_q_mshape_tr = { q_sval_tr, q_cols };
//QSDArray<2> vt_merge_tr(vt_merge.q(), vt_q_mshape_tr);
  QSDArray<2> vt_merge_tr(vt_merge.q(), make_array( q_sval_tr, q_cols));
