...
private:
  ...
  void serialize (Archive& ar, const unsigned int version) { ar & qnum_ & qnum_shape_; }
...
public:
  ...
  typedef std::vector<qnum_array_type> qnum_shape_type;
  ...
  {
    qnum_type qsum_ = qnum_shape_[0][idx[0]];
    for(size_t i = 1; i < qnum_shape_.size(); ++i) qsum_ *= qnum_shape_[i][idx[i]];
    return qsum_.involve(qnum_);
  }
