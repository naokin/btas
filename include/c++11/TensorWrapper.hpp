...
public:
  ...
  : start_(first), tensor_stride_(ext)
  { finish_ = start_+tensor_stride_.size(); }

  /// Shallow copy
  explicit
  TensorWrapper (const TensorWrapper& x)
  : start_(x.start_), finish_(x.finish_), tensor_stride_(x.tensor_stride_)
  { }

  /// Shallow copy from Tensor object
  explicit
  TensorWrapper (Tensor<T,N,Order>& x)
  : start_(x.data()), finish_(x.data()+x.size()), tensor_stride_(x.extent())
  { }
