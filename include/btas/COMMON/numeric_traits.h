...
/// case max(M, N) == M
template<size_t M, size_t N> struct rank_diff<M, N, true>  { const size_t value = M-N; };
/// case max(M, N) == N
template<size_t M, size_t N> struct rank_diff<M, N, false> { const size_t value = N-M; };
