...
//  if(z.dshape() != z_dn_shape)
    for(size_t k = 0; k < N; ++k)
      if(!_dsum_check_dshape(z.dshape(k), z_dn_shape[k]))
        BTAS_ASSERT(false, "btas::STdsum: array shape of z mismatched");
  }
  else {
  ...
  for(typename STArray<T, N>::const_iterator iy = y.begin(); iy != y.end(); ++iy) {
    ...
    for(size_t i = 0; i < N; ++i) y_index[i] += x_shape[i];
...
//  if(x_dn_shape[tk] != y_dn_shape[tk])
    if(!_dsum_check_dshape(x_dn_shape[tk], y_dn_shape[tk]))
      BTAS_ASSERT(false, "btas::STdsum: found mismatched shape to be traced");
  }
  ...
  for(size_t i = 0; i < N-K; ++i) z_dn_shape[dsum_index[i]].insert(z_dn_shape[dsum_index[i]].end(), y_dn_shape[dsum_index[i]].begin(), y_dn_shape[dsum_index[i]].end());
  // Check sparse shape of z
  if(z.size() > 0) {
//  if(z.dshape() != z_dn_shape)
    for(size_t k = 0; k < N; ++k)
  ...
  for(typename STArray<T, N>::const_iterator iy = y.begin(); iy != y.end(); ++iy) {
    IVector<N> y_index(y.index(iy->first));
    for(size_t i = 0; i < N-K; ++i) y_index[dsum_index[i]] += x_shape[dsum_index[i]];
    z.insert(y_index, *iy->second); // Deep copy
  }
}
