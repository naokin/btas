...
public:
 ...
 ~TensorIterator ()
  ...
  : start_(p), index_(idx), tensor_stride_(ext)
  {
  ...
  : start_(p), index_(idx), tensor_stride_(ext), stride_hack_(str)
  {
    current_ = this->get_address(index_);
  }
  ...
  : start_(x.start_), current_(x.current_), index_(x.index_), tensor_stride_(x.tensor_stride_), stride_hack_(x.stride_hack_)
  { }
  ...
  const typename extent_type::value_type& extent (const size_t& n) const { return tensor_stride_.extent(n); }

  /// \return stride of indices
  const stride_type& stride () const { return tensor_stride_.stride(); }

  /// \return nth stride 
  const typename stride_type::value_type& stride (const size_t& n) const { return tensor_stride_.stride(n); }
  ...
  ordinal_type ordinal () const { return tensor_stride_.ordinal(index_); }
  ...
  {
    return *(this->get_address(tensor_stride_.index(this->ordinal()+n)));
  }
...
private:
  ...
  {
    index_ = tensor_stride_.index(this->ordinal()+n);
    current_ = this->get_address(index_);
    return current_;
  }
