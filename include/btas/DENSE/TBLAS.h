...
#include <btas/DENSE/shape_contract.h>
...
{
   BTAS_ASSERT(x.size() == y.size(), "CopyR: x and y must have the same size.");
...
void Axpy (const T& alpha, const TArray<T, N>& x, TArray<T, N>& y)
{
   ...
   if(y.size() > 0)
   {
      BTAS_ASSERT(x.shape() == y.shape(), "Axpy(DENSE): x and y must have the same shape.");
   }
   else
   {
      y.resize(x.shape());
...
{
   BTAS_ASSERT(x.shape() == y.shape(), "Dot(DENSE): x and y must have the same shape.");
...
{
   BTAS_ASSERT(x.shape() == y.shape(), "Dotu(DENSE): x and y must have the same shape.");
...
{
   BTAS_ASSERT(x.shape() == y.shape(), "Dotc(DENSE): x and y must have the same shape.");
            ...
            TArray<T, M-N>& y)
{
   ...
   IVector<M-N> shapeY;
   Gemv_shape_contract(transA, a.shape(), x.shape(), shapeY);

   if(y.size() > 0)
   {
      BTAS_ASSERT(y.shape() == shapeY, "Gemv(DENSE): y must have the same shape as [ a * x ].");
   }
   else
   {
      y.resize(shapeY);
            ...
            TArray<T, M+N>& a)
{
   ...
   IVector<M+N> shapeA;
   Ger_shape_contract(x.shape(), y.shape(), shapeA);

   if(a.size() > 0)
   {
      BTAS_ASSERT(a.shape() == shapeA, "Ger(DENSE): a must have the same shape as [ x ^ y].");
   }
   else
   {
      a.resize(shapeA);
            ...
            TArray<T, N>& c)
{
   ...
   IVector<K> traced;
   IVector<N> shapeC;
   Gemm_shape_contract(transA, transB, a.shape(), b.shape(), traced, shapeC);

   if(c.size() > 0)
   {
      BTAS_ASSERT(c.shape() == shapeC, "Gemm(DENSE): c must have the same shape as [ a * b ].");
   }
   else
   {
      c.resize(shapeC);
   ...
   size_t colsA = std::accumulate(traced.begin(), traced.end(),       1ul, std::multiplies<size_t>());
...
struct __T_dimm_helper<M, N, true> /* (general matrix) x (diagonal matrix) */
{
   ...
   static void call (TArray<T, M>& a, const TArray<U, N>& b)
   {
      ...
      BTAS_ASSERT(b.shape() == shapeB, "Dimm(DENSE): b must have the same shape of column ranks of a.");
...
struct __T_dimm_helper<M, N, false> /* (diagonal matrix) x (general matrix) */
{
   ...
   static void call (const TArray<T, M>& a, TArray<U, N>& b)
   {
      ...
      BTAS_ASSERT(a.shape() == shapeA, "Dimm(DENSE): a must have the same shape of column ranks of b.");
...
struct __T_BlasContract_helper<L, M, N, CALL_GEMV>
{
   template<typename T>
   static void call (
      const CBLAS_TRANSPOSE& transA,
...
/// Case Gemv (B * A)
template<size_t L, size_t M, size_t N>
struct __T_BlasContract_helper<L, M, N, CALL_GEMVT>
{
   template<typename T>
   static void call (
      const CBLAS_TRANSPOSE& transA,
...
/// Case Ger
template<size_t L, size_t M, size_t N>
struct __T_BlasContract_helper<L, M, N, CALL_GER>
{
   template<typename T>
   static void call (
      const CBLAS_TRANSPOSE& transA,
